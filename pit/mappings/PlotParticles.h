...
#ifndef PARTICLES_PIT_MAPPINGS_PlotParticles_H_
#define PARTICLES_PIT_MAPPINGS_PlotParticles_H_
...
/**
 * Plot the particles stored in the leaves
 *
 * This is a plain particle plotter. It has
 *
 * - a vertex write to write down the particles' position,
 * - a cell writer, and
 * - a vertex data writer streaming for each particle its velocity.
 *
 * The cell write is required for technical reasons. It streams per
 * vertex that the only grid entity assigned to this vertex is a
 * point primitive - there are no quadrangles, cubes, ...
 * 
 * Please note that the pit code's move mapping can moving particles
 * beyond their cell's boundaries, i.e. it can move particles out of
 * their cell. The lift process then transports the particle up in the
 * tree until the particle fits into a coarser cell. The latter process
 * terminates on the spacetree's root cell if a particle has left the
 * domain due to the move process. Those particles end up on the
 * spacetree's root. Here, we apply reflecting boundary conditions
 * before we drop them back into the spacetree. This is done in the
 * drop mapping. As a result, we may never call PlotParticles after the
 * move process - it would visualise particles where the reflection has
 * not been applied yet.
 *
 * @author Tobias Weinzierl
 */
class particles::pit::mappings::PlotParticles {
  private:
    /**
    ...
    /**
     * We use a static counter that we append to each output file name. This
     * way, we enable time series, i.e. if this mapping is called several times
     * in a row, it writes file0.vtk, file1.vtk, and so forth. See
     * endIteration() where this static field both is used and incremented.
     */
    static int  _plotCounter;

    tarch::plotter::pointdata::vtk::VTKTextFileWriter*          _vtkWriter;
    tarch::plotter::pointdata::Writer::PointDataWriter*           _particleVelocityWriter;
  public:
    /**
     ...
     */
    PlotParticles(const PlotParticles& masterThread);
    #endif

    /**
     * Destructor. Typically does not implement any operation.
     */
    virtual ~PlotParticles();
     ...
     */   
    void mergeWithWorkerThread(const PlotParticles& workerThread);
    #endif
 ...
 \endcode
     ...
     *                            node. As it is a copy, it is const, i.e. you 
     *                            should not alter it.
