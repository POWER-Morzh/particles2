...
#ifndef PARTICLES_PIDT_MAPPINGS_InitGrid_H_
#define PARTICLES_PIDT_MAPPINGS_InitGrid_H_
...
/**
 * Set up initial grid without assigning any data to it
 * 
 * This mapping either plugs into createVertex (serial mode) or touchLastTime
 * (mpi) to set up the computational grid. In those events, it analyses the
 * given experimental setup and calls refine on the vertex is necessary. As
 * the parallel code refined in touchLastTime and not throughout the
 * construction, it adds at most one level per traversal. This makes the load
 * balancing simpler. In the serial case, the whole grid is built up in one
 * rush. The grid construction is embarassingly parallel.
 *
 * @author Bart Verleye, Tobias Weinzierl
 */
class particles::pidt::mappings::InitGrid {
  private:
    /**
    ...
    static bool                 _haveWrittenWarnings;
  ...
  public:
     ...
     *
     * Copy state attribute from master thread.
     */
    InitGrid(const InitGrid& masterThread);
    #endif
     ...
     */   
    void mergeWithWorkerThread(const InitGrid& workerThread);
    #endif
 ...
 if ( fineGridVertex.isAdjacentToDomainOf( tarch::parallel::Node::getInstance().getRank() ) ) {
   ...
     ...
     *
     * This very operation just builds up the grid. As the grid construction
     * does not do anything, we could go along without any restriction.
     * However, if we switch off restriction, we cannot load balance anymore.
     * As a result, I make this operation return true all the time (we wanna
     * load balance in particular throughout the construction).
     */
