...
#ifndef PARTICLES_PIDT_MAPPINGS_PlotParticles_H_
#define PARTICLES_PIDT_MAPPINGS_PlotParticles_H_
...
/**
 * Plot particles stored within the dual cells that is vertices
 *
 * This plotter runs over the spacetree and plots all the particles
 * assigned to the vertices. The plotting technically is straightforward
 * (consult also the pit mapping for details), but the moment when to
 * plot has to be chosen carefully:
 *
 * The pidt uses the descend events in DropParticles to sort particles
 * into their respective dual cells, and it updates the particle
 * positions in enterCell(). As a result, one has to plug into the
 * events called when a vertex is touched for the last time to get a
 * valid snapshot of the particles.
 *
 * Within touchVertexLastTime() we have to take into account all levels,
 * as the lift mapping plugs into this event as well. If the user makes
 * lift precede the plotting, some particles are already moved to coarser
 * levels and miss in the visualisation consequently.
 * 
 * !!! Shared memory parallelisation
 *
 * This adapter does not work in a multithreaded environment as it writes to
 * streams. I hence disable shared memory parallelisation.
 *
 *
 * @author Bart Verleye, Tobias Weinzierl
 */
class particles::pidt::mappings::PlotParticles {
  private:
    /**
    ...
    tarch::plotter::griddata::Writer::VertexDataWriter*                             _particleVelocityWriter;
  public:
    /**
