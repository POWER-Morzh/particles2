...
#ifndef PARTICLES_PIDT_MAPPINGS_PlotDualGrid_H_
#define PARTICLES_PIDT_MAPPINGS_PlotDualGrid_H_
 ...
 *
 * This adapter works sequentially as it writes to a stream. I thus disable
 * shared memory parallelisation.
 * 
 *
 * @author Bart Verleye, Tobias Weinzierl
 */
class particles::pidt::mappings::PlotDualGrid {
  private:
    /**
  ...
  public:
     ...
     */
    PlotDualGrid(const PlotDualGrid& masterThread);
    #endif

    /**
     * Destructor. Typically does not implement any operation.
     */
    virtual ~PlotDualGrid();
     ...
     */   
    void mergeWithWorkerThread(const PlotDualGrid& workerThread);
    #endif
